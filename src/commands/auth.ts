  ...
  while (i < args.length) {
    ...
    switch (flag) {
      ...
      case '--token-endpoint-auth-method':
        ...
        i += 2; break;
      case '--holder':
        out.holder = requireValue().trim() || undefined;
        i += 2; break;
      case '--takes-holders': {
        const v = requireValue();
        const holders = v.split(',').map(s => s.trim()).filter(Boolean);
        out.takesHolders = holders.length > 0 ? holders : ['world'];
        i += 2; break;
      }
      default:
...
async function registerClient(name: string, args: string[]) {
  if (!name) {
    console.error('Usage: auth register-client <name> [--grant-types G] [--scopes S] [--source SOURCE] [--federated-read SRC1,SRC2,...] [--holder H] [--takes-holders H1,H2] [--redirect-uri URI ...] [--token-endpoint-auth-method client_secret_post|client_secret_basic|none]');
    process.exit(1);
  }
  ...
  const { grantTypes, scopes, sourceId, federatedRead, redirectUris, tokenEndpointAuthMethod, holder, takesHolders } = parsed;
  ...
  try {
    await withConfiguredSql(async (sql) => {
      ...
      const { clientId, clientSecret } = await provider.registerClientManual(
        name, grantTypes, scopes, redirectUris, sourceId, federatedRead, tokenEndpointAuthMethod, { holder, takesHolders },
      );
